...
(function($applozic, w, d) {
                ...
                switch (appOptions) {
                   ...
                   case 'reset':
                    ...
                    case 'getTotalUnreadCount':
                        ...
                        break;
                    case 'getUserActiveChat':      // JVRA
                        return oInstance.getUserActiveChat();
                        break;
    ...
    function Applozic(appOptions) {
        ...
        _this.getUserActiveChat = function() {                  //JVRA
            return MCK_USER_ACTIVE_CHAT;
        };        
        ...
        _this.stoptimer = function() {
            ...
            _this.init = function() {
                ...
                $applozic(d).bind("click", function(e) {
                    ...
                    //console.log( $mck_msg_inner.data('mck-id') );               //jvra
                    MCK_USER_ACTIVE_CHAT = $mck_msg_inner.data('mck-id');        //jvra
                    //console.log( MCK_USER_ACTIVE_CHAT );
...
_this.sendVideoCallMessage = function(callId, msgType, contentType, audioOnly) {
        ...
        function MckMessageLayout() {
            ...
            var contactbox = '<li id="li-${contHtmlExpr}" class="person ${contIdExpr}" data-mck-id="${contIdExpr}" data-isgroup="${contTabExpr}" data-mck-conversationid="${conversationExpr}" data-msg-time="${msgCreatedAtTimeExpr}"><div class="mck-row">' + '<div class="blk-lg-3"><span class="icon">{{html contImgExpr}}</span></div>' + '<div class="blk-lg-9"><div class="mck-row"><div class="blk-lg-8 name" style="color: #ed3737;">${contNameExpr}</div>' + '<div class="blk-lg-4 time mck-truncate">${msgCreatedDateExpr}</div></div>' + '<div class="mck-row"><div class="blk-lg-8 mck-cont-msg-wrapper preview msgTextExpr" style="color:#000000;"></div>' + '<div class="blk-lg-4 mck-unread-count-box unreadcount ${contUnreadExpr}"><span class="mck-unread-count-text text">{{html contUnreadCount}}</span></div></div></div></div></div>' + '</li>';
            ...
            _this.loadTab = function(params, callback) {
                ...
                if (params.tabId) {
                    if ($applozic('.person[data-mck-id ="' + params.tabId + '"][data-isgroup ="' + params.isGroup + '"]').length == 0) {
						...
						_this.updateRecentConversationList(  params.isGroup ? mckGroupUtils.getGroup(params.tabId) 	: false, undefined, true, params.prepend);///: _this.fetchContact(params.tabId), undefined, true, params.prepend);
                    }
        ...
        function MckGroupLayout() {
            ...
            var groupContactbox = '<li id="li-gm-${contHtmlExpr}" class="${contIdExpr} mck-li-group-member" data-mck-id="${contIdExpr}" data-role="${roleVal}" data-alpha="${contFirstAlphaExpr}">' +
                '<div class="mck-row mck-group-member-info" xx1 title="${contNameExpr}">' +
            ...
            var groupMemberSearchContact = '<li id="li-${contHtmlExpr}" class="${contIdExpr} mck-li-group-member" data-mck-id="${contIdExpr}">' + '<a class="mck-add-to-group" href="#" data-mck-id="${contIdExpr}">' + '<div class="mck-row" xx2 title="${contNameExpr}">' + '<div class="blk-lg-3">{{html contImgExpr}}</div>' + '<div class="blk-lg-9">' + '<div class="mck-row"><div class="blk-lg-12 mck-cont-name mck-truncate"><strong>${contNameExpr}</strong></div></div>' + '<div class="mck-row"><div class="blk-lg-12 mck-truncate mck-last-seen-status" title="${contLastSeenExpr}">${contLastSeenExpr}</div></div>' + '</div></div></a></li>';
